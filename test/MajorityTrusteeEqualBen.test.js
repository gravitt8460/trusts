...
const compiledMTEB = require('../build/MajorityTrusteeEqualBen.json');
...
describe('Majority Trustee Multi Beneficiary Contract', () => {
    ...
    describe ('Basics', () => {
        ...
        before(async () => {
            ...
            trust = await new web3.eth.Contract(JSON.parse(compiledMTEB.interface))
                .deploy({
                    data: compiledMTEB.bytecode,
        ...
        it('add beneficiary', async () => {
            ...
            assert.equal(
                await trust.methods.beneficiaries(0).call(),
                accounts[9]
            );
        });
    ...
    describe('Advanced', () => {
        ...
        it('Beneficiaries receive correct amounts', async () => {
            ...
            await trust.methods.requestWithdrawal(String(withdrawalPercent)).send ({
                from: accounts[0]
            });

            await trust.methods.approveWithdrawal(await trust.methods.getWithdrawalRequestCount().call()-1).send ({
                from: accounts[0]
            });

            await trust.methods.finalizeWithdrawal(await trust.methods.getWithdrawalRequestCount().call()-1).send ({
                from: accounts[0]
            });

            payoutPerBen = parseInt(web3.utils.toWei(String(etherToSend / numBens), 'ether') * withdrawalPercent / 100 );
            ...
            assert (afterBalance6 > expectedBalance6 * threshold && afterBalance6 <= expectedBalance6);
            assert (afterBalance7 > expectedBalance7 * threshold && afterBalance7 <= expectedBalance7);
            assert (afterBalance8 > expectedBalance8 * threshold && afterBalance8 <= expectedBalance8);
            assert (afterBalance9 > expectedBalance9 * threshold && afterBalance9 <= expectedBalance9);
        });
        ...
        it('Attempt withdrawal from unregistered trustee', async () => {
            ...
            try {
                // 1- request withdrawal from an unregistered address
                await trust.methods.requestWithdrawal(String(withdrawalPercent)).send ({
                    from: accounts[5]
                });
        ...
        it('Register new trustee and attempt withdrawal', async () => {
            ...
            // approving from newly registered trustee
            await trust.methods.approveWithdrawal(await trust.methods.getWithdrawalRequestCount().call()-1).send ({
                from: accounts[5]
            });

            // approving from owner trustee
            await trust.methods.approveWithdrawal(await trust.methods.getWithdrawalRequestCount().call()-1).send ({
                from: accounts[0]
            });
        ...
        it('Register and unregister trustee, then attempt withdrawal', async () => {
             // 1- register trustee
             await trust.methods.registerTrustee(accounts[4]).send ({
                from: accounts[0]
            });

            // 2- register trustee
            await trust.methods.unregisterTrustee(accounts[4]).send ({
                from: accounts[0]
            });
